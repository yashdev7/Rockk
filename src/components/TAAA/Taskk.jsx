...
import { insertItem, getItems, updateItem, deleteItem } from './services2';
import HeaderLayout from '../Layouts/HeaderLayout';

const GridContext = React.createContext({});

const CommandCell = props => {
    const { enterEdit, remove, add, discard, save, update, cancel, editField } = React.useContext(GridContext);
...
const Task = () => {
    ...
    const enterEdit = dataItem => {
        let newData = data.map(item => (item.id === dataItem.id ? { ...item, inEdit: true } : item));
        setData(newData);
    };
    ...
    return (
        <HeaderLayout>
            <GridContext.Provider value={{ enterEdit, remove, add, discard, update, cancel, editField }}>
                <Grid data={data} onItemChange={itemChange} editField={editField} dataItemKey={'id'}>
                    ...
                    <Column field="project" title="Project Name" width="200px" />
